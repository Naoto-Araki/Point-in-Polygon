...
buildings_shp_path = "博多区建物ポリゴン_2010.shp"
...
# 町丁目の識別列（Name）と建物の識別列（gml_id）の確認
if "gml_id" not in buildings_gdf.columns:
    raise ValueError("建物ポリゴンに 'gml_id' 列がありません。")
...
# 3. 各建物の gml_id ごとに最大面積を持つ町丁目を選択
max_area_gdf = intersection_gdf.loc[intersection_gdf.groupby("gml_id")["area"].idxmax()]

# 4. 元の建物データに最大面積の町丁目情報（Name列）を統合
result_gdf = buildings_gdf.merge(
    max_area_gdf[["gml_id", "Name"]], on="gml_id", how="left"
)

# 結果の保存
output_path = "博多区建物ポリゴン_2010_町丁目付加.shp"
